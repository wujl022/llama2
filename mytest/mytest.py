...
class Task:
    ...
    def message_put(self):
        if len(self.b) > len(self.a):
            t = self.b
            self.message.put(self.b.replace(self.a, ''))
            self.a = t
                    ...
                    while self.lock.locked():
                        if self.has_higher_priority_task(priority):
                            ...
                            if done_time >= min_time_slice:
                                ...
                                self.lock_timer.cancel()  # 取消计时器
                                # 启动锁的计时器
                                self.lock_timer = Timer(self.time_slice[priority],
                                                        lambda: self.save_content_preempt(task))
                                self.lock_timer.start()
